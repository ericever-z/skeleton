...
module.exports.create = function(info){
	...
	console.log("如果发现下载很慢请使用镜像，输入命令如下");//指向taobao npm 镜像");
	console.log("npm install -g cnpm --registry=https://registry.npm.taobao.org");
    console.log("☞ ① 进入项目文件夹： " + project.name);
	cd(project.name);
    console.log("☞ ② 执行命令 npm install &&  bower install 构建项目 ....");
